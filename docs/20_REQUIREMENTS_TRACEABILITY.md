...
|---|---|---|---|---|
| FR-001 Point/Cell | `packages/core` | `core.test.ts`、PostGIS integration | G1/G2/G4 | SDK PASS；DB Adapter 13/13 slice PASS；G4 PARTIAL |
| FR-002 Geometry/Cells | `packages/geometry` | `geometry.test.ts`、PostGIS integration | G1/G2/G4 | SDK PASS；Antimeridian Polygon/MultiPolygon DB slice PASS；G4 PARTIAL |
...
| FR-005 Aggregation | `packages/aggregation`、SQL | `analysis.test.ts`、Benchmark、SQL assertions | G1/G3/G4 | Node PASS；四列 Upsert slice PASS；G4 PARTIAL |
...
| FR-010 PostGIS | `packages/postgis`、`database/` | Golden/integration/smoke/EXPLAIN/scale/lifecycle/restore | G4 | 个别严格切片通过至 10M；完整 run-scoped G4 PARTIAL |
| FR-011 Web Demo | `apps/web-demo`、`browser-tests/` | build/browser matrix/Axe | G2/G6 | Build/Budget PASS；Chromium/Firefox/WebKit 9/9 PASS；跨平台 CI PARTIAL |
...
| NFR-007 可移植 | Node/Browser/PostGIS/Adapter | 6、7、23 章；P1-CROSS-PLATFORM | G1/G6 | Windows Node 22 本地与三浏览器 PASS；托管 Linux/macOS/Windows/arm64 矩阵未执行，PARTIAL |
| NFR-008 可恢复 | Backup/PITR/Job 幂等 | 15、23 章；P1-DEPLOY-DR/P2-JOB | G6 | 本地 backup/restore/lifecycle 切片通过；完整 G4、PITR、HA/Failover NOT_RUN |
...
| NFR-011 供应链 | License、SBOM、签名、provenance | P1-SUPPLY | G5/G7 | Source SBOM 145 components / 134 external packages、License 146 production packages PASS；source-label 绑定前镜像扫描 3 Critical + 19 High 且阈值 FAIL，不能绑定当前 commit；signing/provenance NOT_RUN |
